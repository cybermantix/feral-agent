...
#[CatalogNodeDecorator(
    ...
    name: 'Merge Strings',
    group: 'Data',
    description: 'Merge multiple paths into one string context variable.'
)]
class MergeStringsNodeCode implements NodeCodeInterface
{
    ...
    const KEY = 'merge_strings';

    const NAME = 'Merge Strings';

    const DESCRIPTION = 'Take an array of context paths and merge into one context variable.';
    ...
    public function process(ContextInterface $context): ResultInterface
    {
        ...
        foreach ($inputPaths as $path) {
            $contextValue = $this->getValueFromContext($path, $context);
            if (is_string($contextValue)) {
                $outputData[] = $contextValue;
            } else if (is_array($contextValue)) {
                $outputData[] = implode("\n", $contextValue);
            } else {
                $outputData[] = (String) $contextValue;
            }
        }
